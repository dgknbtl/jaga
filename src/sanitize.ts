...
/** Tags whose entire content is dangerous—strip tag + inner content. */
const RAWTEXT_TAGS = new Set(['script', 'style', 'iframe', 'object', 'embed', 'base']);

/** Safe URL protocols (everything else is blocked). */
const SAFE_PROTOCOLS = /^(https?|mailto|tel|ftp):/i;

/** URL attributes that must be protocol-checked. */
...
function safeUrl(value: string): string {
  ...
  // Decode HTML entities and check protocol
  ...
  return escapeAttrValue(trimmed);
}
...
function buildAttrs(
  ...
  for (const [name, value] of Object.entries(rawAttrs)) {
    ...
    const safe = URL_ATTRS.has(name) ? safeUrl(value) : escapeAttrValue(value);
    out += ` ${name}="${safe}"`;
  }
  return out;
}
...
function parse(
  ...
  let textBuf = '';
  let rawDepth = 0;   // depth counter for rawtext tags
  let rawTag = '';    // current rawtext tag name
  ...
  const emitTag = () => {
    ...
    parts.push(selfClosing ? `<${name}${attrStr}>` : `<${name}${attrStr}>`);
  };
  ...
  for (let i = 0; i < html.length; i++) {
    ...
    switch (state) {
      ...
      case 'CLOSING_TAG':
        if (ch === '>') {
          ...
          if (RAWTEXT_TAGS.has(name) && rawTag === name) {
            rawDepth--;
            if (rawDepth === 0) { state = 'TEXT'; rawTag = ''; }
          } else {
            if (allowedTags.has(name)) parts.push(`</${name}>`);
            state = 'TEXT';
          }
        ...
        else if (/[\s]/.test(ch)) { /* ignore whitespace */ }
        else tagName += ch;
        break;
      ...
      case 'ATTRS':
        ...
        else if (ch === '/') { /* self-closing slash, ignore */ }
 ...
 * Works in Node.js, Bun, Deno, and browsers — zero dependencies.
 *
 * @example
 * import { sanitize } from 'jagajs/sanitize';
 * const clean = sanitize(richTextEditorOutput);
 *
 * @example
 * // With Jaga core
 * import { j } from 'jagajs';
 * import { sanitize } from 'jagajs/sanitize';
 * const html = j`<article>${sanitize(userContent)}</article>`;
 */
export function sanitize(html: string, options: SanitizeOptions = {}): JagaHTML {
  const allowedTags = new Set(
    (options.allowedTags ?? DEFAULT_TAGS).map((t) => t.toLowerCase())
  );
  ...
  return new JagaHTML(parse(html, allowedTags, mergedAttrs));
}
